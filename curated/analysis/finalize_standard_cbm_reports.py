...
def main() -> None:
    ...
    if missing:
        raise RuntimeError(
            f"ERROR: {path.name} is incomplete; expected PNG output from "
            f"{len(expected)} figure cells, missing cells {missing}"
        )
