...
| 5 | CUB/CUB70: does the mechanism transfer to real birds? | CUB70-CBM exploration executed; natural visibility/species approximations exist; reciprocal whole-part deletion and two small-patch calibrations were documented failures | **OBSERVATIONAL CONVERGENCE; EDIT-BASED CAUSAL CLAIMS QUARANTINED**. Patch V2 passed 5/6 checks but had an unfair mean-fill wing control and only 11 surviving wing images | Pair each accepted FunnyBird question with CUB's closest approximation and explicit limitation; do not let the failed patch proxy block the remaining chain. Calibrated 2-D swap is last | After CBM story |
