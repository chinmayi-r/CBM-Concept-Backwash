...
| 5 | CUB/CUB70: does the mechanism transfer to real birds? | CUB70-CBM exploration executed; mask omissions/collapse audited; reciprocal whole-part deletion ran; randomized small-patch FunnyBird calibration ran | **OBSERVATIONAL CANDIDATE; CAUSAL TESTS QUARANTINED**. Whole-part edits were visibly invalid; small patches showed selected local raw-`z` responses but failed calibration, omitted wing, and correctly stopped before CUB70 | Inspect the saved patch-selection audit, then predeclare one corrected FunnyBird raw-`z` calibration before any CUB causal claim | After CBM story |
