...
echo "[1/6] Verify final Koh Standard and fixed-swap manifests"
...
echo "[2/6] Revalidate the accepted renderer-swap files"
python analysis/validate_fixed_swaps.py --out "$SWAP_ROOT"

echo "[3/6] Derive corrected bilateral target visibility; no inference or training"
python analysis/derive_funnybird_visibility.py \
  --swap-root "$SWAP_ROOT" \
  --output "$VISIBILITY_ROOT"

echo "[4/6] Rebuild notebook 02 from its versioned builder"
python analysis/build_standard_cbm_reports.py --only 02

echo "[5/6] Execute notebook 02 in place; this reads completed artifacts and submits no jobs"
jupyter nbconvert --to notebook --execute --inplace \
  --ExecutePreprocessor.timeout=-1 \
  notebooks/02_funnybirds_cbm.ipynb

echo "[6/6] Export standalone HTML and restore figure alt text"
