#!/usr/bin/env bash
# Entry 3: submit exactly full-CUB standard seed 1.
set -euo pipefail
: "${CURATED_DATA:?export CURATED_DATA}"
REPO="${REPO:-$(git rev-parse --show-toplevel)}"
JOB=koh_resnet_cub_s1
...
echo "===== ENTRY 3: FULL CUB STANDARD S1 ====="
...
echo "concepts=112 species=200 raw-logit linear class head"
...
jid=$(sbatch --parsable --time=1-00:00:00 --job-name="$JOB" \
  --export="ALL,REPO=$REPO,CURATED_DATA=$CURATED_DATA,DATASET=cub,LABELS=standard,SEED=1,BACKBONE=resnet50,KOH_TRAINING_PROTOCOL=koh_original,KOH_OUTPUT_ROOT=$ROOT" \
  "$REPO/curated/train/koh_joint_job.slurm")
echo "[ENTRY 3 SUBMITTED] job=$jid dependency=none output=$OUT"
