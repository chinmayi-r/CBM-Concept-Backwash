#!/usr/bin/env bash
# Entry 5: continue FunnyBird Standard seed 1 from epoch 100 until stable.
set -euo pipefail
: "${CURATED_DATA:?export CURATED_DATA}"
REPO="${REPO:-$(git rev-parse --show-toplevel)}"
JOB=koh_fb_standard_converge_s1
OUT="$CURATED_DATA/koh_joint_resnet_accelerated_converged_v1/funnybirds/standard/seed1"
...
echo "===== ENTRY 5: FUNNYBIRD STANDARD S1 CONVERGENCE CONTINUATION ====="
...
echo "goal=resume accepted Standard seed 1; test stability every 25 epochs; cap at 200"
echo "framework=Koh Joint CBM backbone=ResNet-50 labels=standard seed=1"
...
echo "COMMAND: sbatch --job-name=$JOB --export=ALL,REPO=$REPO,CURATED_DATA=$CURATED_DATA,LABELS=standard curated/train/koh_funnybird_convergence_job.slurm"
jid=$(sbatch --parsable --job-name="$JOB" \
  --export="ALL,REPO=$REPO,CURATED_DATA=$CURATED_DATA,LABELS=standard" \
  "$REPO/curated/train/koh_funnybird_convergence_job.slurm")
echo "[ENTRY 5 SUBMITTED] job=$jid dependency=none output=$OUT"
