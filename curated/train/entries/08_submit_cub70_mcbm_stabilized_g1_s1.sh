#!/usr/bin/env bash
# Entry 8: CUB70 stabilized high-gamma bridge, gamma 1, seed 1.
set -euo pipefail
: "${CURATED_DATA:?export CURATED_DATA}"
REPO="${REPO:-$(git rev-parse --show-toplevel)}"
JOB=m_cub70_stable_g1_s1
OUT="$CURATED_DATA/mcbm_stabilized_high_gamma_v1/cub70/standard/g1/seed1"
...
echo "===== ENTRY 8: CUB70 MCBM STABILIZED BRIDGE GAMMA 1 S1 ====="
...
echo "framework=official minimal_cbm MCBM backbone=ResNet-50 dataset=CUB70 seed=1 gamma=1"
...
echo "COMMAND: sbatch --job-name=$JOB --export=ALL,REPO=$REPO,CURATED_DATA=$CURATED_DATA,DATASET=cub70,LABELS=standard,GAMMA=1,SEED=1,MCBM_STABILITY_PROTOCOL=cub70_stabilized_high_gamma_v1,MCBM_TRAINING_PRECISION=fp32,BASE_LR=0.003 curated/train/mcbm_stabilized_job.slurm"

bash "$REPO/curated/train/verify_canonical_sources.sh"
jid=$(sbatch --parsable --job-name="$JOB"   --export="ALL,REPO=$REPO,CURATED_DATA=$CURATED_DATA,DATASET=cub70,LABELS=standard,GAMMA=1,SEED=1,MCBM_STABILITY_PROTOCOL=cub70_stabilized_high_gamma_v1,MCBM_TRAINING_PRECISION=fp32,BASE_LR=0.003"   "$REPO/curated/train/mcbm_stabilized_job.slurm")
echo "[ENTRY 8 SUBMITTED] job=$jid dependency=none output=$OUT"
